...
class RatingBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < 5; i++) {
      if (i < realNumber) {
        starList.add(Icon(Icons.star_rounded, color: Colors.orange, size: size));
    ...
    ratingCount != null ? starList.add(Padding(
      ...
      child: Text('($ratingCount)', style: robotoRegular.copyWith(fontSize: size*0.8, color: Theme.of(context).textTheme.bodyLarge!.color!.withValues(alpha: 0.7)), textDirection: TextDirection.ltr),
    )) : const SizedBox();
