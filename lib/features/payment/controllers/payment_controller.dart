...
class PaymentController extends GetxController implements GetxService {
  ...
  Future<bool> saveOfflineInfo(String data) async {
    _isLoading = true;
    update();
    bool success = await paymentServiceInterface.saveOfflineInfo(data, AuthHelper.isLoggedIn() ? null : AuthHelper.getGuestId());
    _isLoading = false;
    update();
    return success;
  }
  ...
  Future<bool> updateOfflineInfo(String data) async {
    _isLoading = true;
    update();
    bool success = await paymentServiceInterface.updateOfflineInfo(data, AuthHelper.isLoggedIn() ? null : AuthHelper.getGuestId());
    _isLoading = false;
    update();
    return success;
  }
