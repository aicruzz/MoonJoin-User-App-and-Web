...
| `MoonJoinNotificationBanner` / `MoonJoinNotificationData` | `moonjoin/notifications/moonjoin_notification_banner.dart` | **THE official MoonJoin notification presentation layer** — premium in-app foreground banner (OverlayEntry) reusing `MoonJoinStatusAnimation`; the ONLY in-app notification component. All future types (Orders/Trips/Parcel/Wallet/Payment/KYC/Rewards/Promotions/Unavailable) reuse it. No duplicate notification UI ever. **Unavailable variant** (`unavailable:true`) adds warm-in arrival + breathing moon container + amber leading strip + `sweep` Arc-Light; normal banner unchanged |
| `MoonJoinPresentationState` | `moonjoin/moonjoin_presentation_state.dart` | **THE single shared resolver** — `MoonJoinPresentationState.fromOrder(order)` = `{motion, unavailable}`. Owns the ONE "unavailable overrides pending" rule (Home `_UnavailableItemsCard` rule, reused exactly). Consumed by Running Order popup, unavailable watcher, notification_helper. **Never duplicate this rule** |
| `MoonJoinUnavailableWatcher` | `moonjoin/notifications/moonjoin_unavailable_watcher.dart` | Zero-layout state-reactive presenter of the persistent unavailable banner off `OrderController` (same data as the Home card). One session per order, survives refreshes, persistent (never auto-dismiss), dismissal never clears business state, higher priority than normal status. Mounted first in the dashboard `ExpandableBottomSheet` background |
| `MoonJoinOnboardingArt` (`MoonJoinOnboardingScene`, `MoonJoinOnboardingPalette`, `MoonJoinOnboardingPainter`) | `moonjoin/onboarding/moonjoin_onboarding_art.dart` | **THE onboarding illustration system** — original programmatic MoonJoin artwork ("One Orbit" living city at night); 4 scenes (arrival/ecosystem/delivery/resolution). Static layer cached as `ui.Picture`, thin animated light layer, no blur (gradient-shader glows), single ambient. FROZEN — reuse-only, never fork, never raster/stock art |
| `MoonJoinMoonPhaseProgress` | `moonjoin/onboarding/moonjoin_moon_phase_progress.dart` | Onboarding progress rendered as moon phases (crescent→full), replacing dots. Cheap static painter |
...
| **`ItemWidget` owner/provider identity** (FROZEN 2026-08-03) | `common/widgets/item_widget.dart` (normal layout + `_premiumStoreDishCard`) | **THE single owner-identity label.** Outside a Store/Restaurant/Provider page, item cards show the owner from real `item.storeName` (never faked), directly under the item name, muted `disabledColor`, size `(fontSizeExtraSmall+fontSizeSmall)/2` (bigger than before, still < item name), graceful-hide when null. Hidden when `inStore==true` (inside owner pages). Shared by Search/Favorites/Category/All-Store premium. No second identity system; never touch `ItemCard`. |
| **`ModuleTerminology`** (FROZEN 2026-08-03) | `helper/module_terminology_helper.dart` | **THE single source of module-aware header terminology.** `providerLabelPlural(module)` → premium bare labels (Restaurants/Grocery Stores/Pharmacies/Fashion Stores/Markets/Fuel Stations/Drink Distributors); `newOnHeader(module)` → "New {label} on {App}". Prefers future backend `provider_label_plural` (on `ModuleModel`), else moduleType+moduleName fallback. Used by `all_store_screen`, `new_on_mart_view`, `all_store_filter_widget`. Parcel/Rental untouched. **Brands stay Ecommerce-only** (gated in `all_store_screen._topBrands()`) until backend module-scopes `/api/v1/brand`. |
