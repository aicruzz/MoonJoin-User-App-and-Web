...
## Phase 3B / 3G — Edit Unavailable Items: Update Cart + Customer-Edit Contract (pending & confirmed) + Wallet Error — STATUS: FROZEN (2026-08-14) · commit `689d53c` · owner-approved on iPhone
**Supersedes the 2026-08-04 blocker above.** Resolved end-to-end and runtime-verified on the owner's iPhone (orders **#100098** confirmed+paid and **#100093** wallet).

- **Update Cart unblocked.** The backend contradiction was fixed on the backend side (commit `f037cec` `is_array()` guard) and is **fingerprint-proven deployed in production** (the `admin/order/emergency-status-override` route added by `35d3c73` returns HTTP **405** live; controls 405/404 validate the method — so production runs ≥`35d3c73`, which includes `f037cec` + the `52dd41e` customer-edit contract). Frontend sends **`'cart': cart` (native JSON array)** through the single `putData` `jsonEncode(body)`. **Device-verified `PUT /api/v1/customer/order/update/100098` → HTTP 200 "Order updated successfully".** No `jsonEncode(cart)`, no workaround, no duplicate API.
- **Three-field contract (authoritative, permanent):** `unavailable_item_note` = customer checkout instruction (never a vendor signal, never overwritten); `customer_edit_requested` (0/1→bool) = the vendor edit-request signal; `unavailable_item_vendor_note` = the vendor shortage message shown in the Edit banner; `unavailable_item_ids` = affected detail IDs. Backend source (repo `aicruzz/admin.moonjoin.com`, read-only audit at HEAD `738d6ee`): `Api/V1/OrderController@update` authorizes customer edit for `order_status ∈ [pending, confirmed]` while unclaimed (payment_status not a gate; `customer_edit_requested` reset to 0 + ids/vendor-note cleared on success); `Vendor/OrderController::markUnavailableItems` sets `customer_edit_requested=1` + vendor note + ids and does **not** change `order_status`.
- **canEdit now surfaces pending AND confirmed:** `(pending && unpaid) || ((pending || confirmed) && customerEditRequested)`. Confirmed editable **only** via the vendor request (never blanket-confirmed, never processing). Home card (`module_landing_view`) + shared resolver (`MoonJoinPresentationState.fromOrder`) use the identical rule. **Verified:** confirmed #100098 → Home card + Edit button + vendor banner "Coca cola is not available, choose other drink" + 200 update + request cleared on refresh.
- **Wallet-insufficient UX:** `updateOrder` now returns `Response` (not bool); `OrderEditController.updateFailureMessage(Response)` shows a specific "Insufficient wallet balance…" message for `403 + code=='wallet'` **only**, generic otherwise. Unit test `test/order/update_order_error_message_test.dart` (4 cases) + device-verified on #100093 (403 → specific message; 200 success path unchanged).
- **Reuse (per docs rule):** the ONE Edit Unavailable Items flow reuses the ONE frozen Product Details for Add/Edit, the shared presentation resolver, and the existing cart calculations/APIs — no duplicate screens or logic. `flutter analyze` clean; no business/payment/claim/status logic changed; backend untouched.
- **Known separate issue (NOT this freeze):** `MoonJoinUnavailableWatcher` `Overlay.of` null crash (`moonjoin_unavailable_watcher.dart:63`) — banner presentation only; card/details/edit/update all work. Tracked for a dedicated later fix.
...
## Flash Deals on Module Home (AllStoreScreen) + Food flash-refresh lifecycle — STATUS: FROZEN (2026-08-15) · owner-approved on iPhone
**Part 1 — Flash section (reuse-only).** Inserted the existing approved `FlashSaleViewWidget` (renders the approved `FlashProductCard` — item-advertising card, not video) into the approved mobile Module Home `AllStoreScreen` (Food/Grocery/Pharmacy/Ecommerce; the legacy `modules/grocery_home_screen.dart`/`shop_home_screen.dart` were NOT touched). Placement: in `_storeList`, after `_topBrands()` and before the provider `ListView` → Promo Banner → [Brand] → Flash → Provider banners (both cases). Gated to the clean home state (`_activeFilter == -1 && _discovery == -1`) like the promo banner; self-hides with no active flash products. Data via the existing `FlashSaleController.getFlashSale(false, false)` in `AllStoreScreen._load()`. No new component/API/model/design.

**Part 2 — Food flash-refresh lifecycle fix.** Symptom: a Food flash-sale order updated the backend sold count but the mounted Restaurants screen stayed stale until app close/reopen, whereas Ecommerce/Fashion updated instantly. Root cause (proven by module comparison): all modules share ONE global `FlashSaleController` + the same `AllStoreScreen` flash card; the only divergence was that `HomeScreen.loadData` — which `CheckoutController` calls as `HomeScreen.loadData(true)` after a successful order (`checkout_controller.dart:520`) — re-fetched the flash sale only for `grocery`/`ecommerce` (`home_screen.dart:69-76`), with no `food`/`pharmacy` branch, so Food's mounted card never refetched post-order. Fix (architecture-consistent): `loadData` now calls the SAME existing `getFlashSale(reload, false)` for `grocery || food || pharmacy || ecommerce`; ecommerce-only extras stay ecommerce-only. Food now follows the identical proven refresh lifecycle as Ecommerce/Fashion.

**Backend unchanged.** Flash Sale reservation/depletion, quantity validation, pricing/discount, and module/zone/per-store scoping are backend-owned and already correct — this was purely a User-App state-refresh lifecycle bug (read-only API probe confirmed the flash data is served per-module and per-store; e.g. Food zone-7 flash-sales served the active Pizza/Shawarma events).

**Files:** `lib/features/store/screens/all_store_screen.dart` (+ flash insertion, imports, `_load` flash fetch) and `lib/features/home/screens/home_screen.dart` (`loadData` flash-refresh gating). `flutter analyze` clean; release build ✓; **physical-iPhone verified & owner-approved**: Food flash sold count updates instantly after an order without close/reopen; Ecommerce + Fashion still instant; Food depletion + Shawarma 2/3 rejection intact.

## Rental Flash Rent — Rental-level flash pricing + placement + label — STATUS: FROZEN (2026-08-16) · owner-approved
**Scope.** ONE Rental-level Flash Sale pricing/presentation pattern for the whole Rental module (apartments are `Vehicle` records distinguished by `RentalApartmentAdapter` category — no separate entity). All pricing resolves by a rentalType STRING key (`hourly`|`day_wise`|`distance_wise`, default → distance) via `VehicleModel.baseRate/applicableRate/hasFlashRate(rentalType)`, so Car Rental works now and a future Short Apt rate key reuses the same logic with no rewrite. The frontend renders backend `flash_sale.prices[axis].flash_price` verbatim; amount/`booking_total` campaigns keep the original per-unit rate (no invented price).

**Placement.** The shared `AllVehicleScreen._flashDeals` section renders BELOW Top Brands (Food/Grocery/Ecommerce pattern) → Banner → Top Brands → Flash Rent → Providers. Same shared build serves BOTH Car Rental Home and Short Apartments Home (apartment mode has no brand section → Flash Rent sits under the banner, the analogous position). Self-hides when no flash vehicles qualify.

**Calculation (the real fix).** `TaxiPriceHelper.calculateTripCost` now uses `applicableRate`, so the flash rate flows through the whole pipeline: cart → checkout `subtotal` → `getTripTax` base → `total` → `trip_amount` booking payload (fixed at source, not overridden at the UI). Flash replaces (never stacks on) the vehicle's own discount; provider promos still apply on the corrected subtotal. `calculateOriginalTripCost` + `calculateFlashDiscount` drive the bill's original-cost + Flash Sale saving lines. Non-flash bookings are byte-identical.

**Card & label.** Home card shows the flash price + struck-through original + % OFF badge (all from backend data). The section heading is "Flash Rent" (new `flash_rent` key in en/es/ar/bn) to distinguish rental promos from the "Flash Sale" wording of Food/Grocery/Ecommerce — a label-only change; the card badge and the bill's Flash Sale line still use `flash_sale`.

**Files:** `vehicle_details_model.dart`, `taxi_price_helper.dart`, `all_vehicle_screen.dart`, `rental_popular_card.dart`, `bill_details_widget.dart`, `taxi_checkout_screen.dart`, `vehicle_details_screen.dart`, `vehicle_card.dart`, `assets/language/{en,es,ar,bn}.json`, `test/rental/taxi_price_helper_flash_test.dart`. `flutter analyze` clean; 24 rental tests pass (6 new flash: per-axis flash rate used, non-flash unchanged, no double-discount); iOS release build + device install ✓.

**Backend unchanged / documented delivery dependency.** The rental flash contract (`Vehicle.$appends['flash_sale']` → `getFlashSaleAttribute` → `RentalFlashSaleVehicle::resolveFor`) is backend-owned. The customer list/cart endpoints (`Api/Public/VehicleController`) eager-load the provider WITHOUT `module_id` (`provider:id,name,address,tax`); the accessor early-returns null when `provider->module_id` is null, so those endpoints return `flash_sale: null` while the details endpoint returns it populated. Until the backend eager-load includes `module_id`, the live Home Flash Rent section + checkout flash line stay empty by correct design (the detail banner/estimate already work); they light up with zero further frontend change once the backend delivers `flash_sale` on the list/cart endpoints.

## MoonJoin Flash Deals UI — reusable premium presentation (Food/Grocery/Fashion + Rental "Flash Rent") — STATUS: FROZEN (2026-08-16) · owner-approved
**Presentation redesign only.** Replaced the legacy image-only Flash Sale PageView + shared below-card block with the approved MoonJoin Flash Deals design (source of truth: `ui-designs/flashcard.PNG` card + `ui-designs/flashcard_sample_look.PNG` home composition). The section is now one intentionally-designed module: organic pale-green wavy background (ClipPath) → header (green lightning disc + title/subtitle, "Ends in" ticking countdown, View All) → premium hero cards (%OFF badge, title, provider + verified, flash price + struck original, 🔥 sold% + progress) → pagination dots.

**Reusable components (new).** `lib/common/widgets/moonjoin/moonjoin_flash_deals_section.dart` (shell + `_FlashOrganicClipper` + `_FlashCountdown`) and `lib/common/widgets/moonjoin/moonjoin_flash_deal_card.dart` (pure-presentation card taking pre-formatted strings only). One flash card/section app-wide — each module maps its own already-working data onto the props; no per-module duplicate, no logic in the card.

**Wiring.** Product Flash Sale (`flash_sale_view_widget.dart`, used by `AllStoreScreen`) maps `ActiveProducts`/`Item` → card, keeping `FlashSaleController`/models, `duration` countdown, `pageIndex`, View All → `RouteHelper.getFlashSaleDetailsScreen`, tap → `ItemController.navigateToItemPage`. Rental Flash Rent (`all_vehicle_screen._flashDeals`) maps `VehicleModel` → card, keeping the label "Flash Rent", `RentalApartmentAdapter` category split, the frozen Rental-level pricing helpers, tap → `VehicleDetailsScreen`; countdown reads `flash_sale.end_date`, View All hidden (no rental destination), sold row/progress hidden (no rental sold data).

**Preserved / untouched.** Backend, API contracts, DB, Admin, Flash Sale engine, discount calculations, stock/sold, navigation, cart/checkout/booking, the `AllStoreScreen` placement + clean-state gating + `getFlashSale` refresh lifecycle, the shimmer/loading + empty state, and desktop/web `WebFlashSaleViewWidget`. New key `ends_in` added to en/es/ar/bn.

**Verification.** Design read from both `ui-designs/` references; `flutter analyze` clean; a rendered-golden self-check confirmed the composition matches and caught a 2.4px card overflow (fixed by making the sold-row label `Flexible`); durable no-overflow layout guard `test/moonjoin/flash_deals_layout_test.dart` passes; 30 tests pass total; iOS release build + device install ✓. Note: live product Flash Deals render immediately (product flash data is delivered); the Rental Flash Rent section still depends on the documented backend list/cart `module_id` delivery to populate on device — presentation is ready and correct either way.

## MoonJoin Flash Deals + View All — refinement & finalization — STATUS: FROZEN (FINAL, 2026-08-17) · owner-approved on device
Refinement + finalization of the Flash Deals presentation (previous entry: reusable section/card). PRESENTATION ONLY — no business logic, API, pricing, stock/sold, navigation or Rental Flash Rent change.

**Card refinements (approved on device).** Flash price → `FittedBox(scaleDown)` (always fully readable, never "₦3,7…"); struck original ellipsises first. Product image → `BoxFit.contain` (full product silhouette, no crop, no distortion), content/image ~48/52. Inner white card is a clean rounded rect — the organic `ClipPath` is painted BEHIND the content in a `Stack` (amoeba on the green background only, never on the card). Carousel auto-rotates every 4s (pauses on touch, resumes); manual swipe, pagination dots, card tap preserved.

**Countdown (the real fix + final format).** DAYS:HRS:MINS:SECS — days a SEPARATE block (never total hours), seconds always ticking. The earlier static bug was a multi-day campaign showing DAYS:HRS:MINS (no seconds) → looked frozen though it was counting. `MoonjoinFlashCountdown` (now public) computes `endTime − now` each second from an ABSOLUTE campaign end (parsed like `FlashSaleController`, UTC→local), so it can't reset/freeze on rebuild and stays in sync with the View All/controller countdown. The 4 blocks sit in a header flex slot with `FittedBox(scaleDown)` so they never overflow. Pure `moonjoinFlashCountdownDigits(Duration)` is unit-tested (days separate, seconds present, rolls, clamps at 00:00:00:00) and a widget test proves it visibly ticks without reset.

**View All page redesign.** `flash_sale_details_screen.dart` now uses the approved MoonJoin `ProfilePageHeader` green header (same as the Favourite page; `CustomAppBar` on desktop) + a centered `MoonjoinFlashCountdown` strip + a responsive list/grid of the approved `MoonjoinFlashDealCard`. Preserved: `getFlashSaleWithId` pagination, item navigation (disabled when sold out), discount/stock/sold, `getStartingPrice`, `FooterView`/`webMaxWidth`. No legacy Flash Sale presentation remains.
...
## MoonJoin Food Top Brands (Approved & Frozen) + Campaigns section + owning-store logo overlay — STATUS: FROZEN (2026-08-18) · owner-approved on iPhone
Two presentation-only features finalized together (they share the two home files). **No backend/API/controller/model/business-logic change.** Reference: `ui-designs/Top_Brands.png` (Top Brands), `ui-designs/campaign.png` (Campaigns).

**Food Top Brands (Food module only).** New reusable `MoonjoinTopBrandCard` + `MoonjoinFoodTopBrands` section, inserted on the active mobile home `all_store_screen.dart` (at the established Top Brands position) and the web home `web_new_home_screen.dart`. Reference-locked to `ui-designs/Top_Brands.png`: simple **rounded-rectangle, near-square** card (`height == width`); **exactly four complete cards** across a phone (responsive `cardW = (screenW − 2·pad − 3·gap)/4`, clamped 70–120; no fifth-card peek); **the vendor-uploaded logo/image itself fills the whole card** via `BoxFit.cover`, rounded-clipped (its own background defines the look — colored logo → colored card, transparent → light KFC/Domino's-style); brand name at the bottom over a scrim; **no heart, no fabricated outlet count, no outer panel**. Data = existing `StoreController.featuredStoreList` (`featured=1`, module/zone-scoped) — no new model/API/source; tap → `getStoreRoute`, See-all → `getAllStoreRoute('featured')`. Module-gated to Food; self-hides when empty. **Ecommerce/Fashion/Rental Top Brands NOT touched** (shared legacy `TopBrandCard` + their section heights unchanged); the approved Food card becomes the reference for later, separately-requested module reuse.

**REJECTED approaches removed/forbidden (Top Brands):** amoeba/blob cards, generated brand-color backgrounds, `palette_generator`/dominant-color extraction, oversized/portrait cards, a small floating logo inside a generated shape, the favourite heart, fabricated counts. **Cleanup:** `palette_generator` removed from `pubspec.yaml`; rejected `moonjoin_brand_blob_card.dart` deleted; audited (`grep`) that no `palette_generator`/`PaletteGenerator`/blob/brand-color-cache Top-Brands artifacts remain in `lib/`, `test/`, or `pubspec*`.

**Campaigns section + restored owning-store logo.** `MoonjoinCampaignSection` + `MoonjoinCampaignCard` render the existing `CampaignController.itemCampaignList` in the approved pale-green organic design; `Item.store` field (defensive try/catch parse). The card's **owning restaurant/vendor logo badge** (lower-left of the product image) is resolved by the new **pure `resolveCampaignStoreLogo`**: **(1)** primary `campaign.store.logo_full_url` (the `with('store')` relation — used automatically once the live `/api/v1/campaigns/item` returns it); **(2)** fallback — the store already loaded for this module home matched by the item's real `storeId`, reusing its `logoFullUrl`. **Why it had disappeared:** the card relied solely on `item.store`, which the current live campaign endpoint does not return → null → badge omitted; the fallback restores the REAL owning-store logo from existing loaded data (no request, no name match, no placeholder, no generated/hardcoded logo, no Top-Brands/palette logic) and gracefully omits when unavailable. Campaign design otherwise **unchanged**.

**Verification.** `flutter analyze` clean on all changed files (35 total repo issues = pre-existing, unchanged, none in these files). Deterministic tests pass: `campaign_test.dart` **9** (store parse ×3 + `resolveCampaignStoreLogo` ×4 + section self-hide/loading ×2), advertisement 5, flash layout 1. The `flash_countdown_test` "visibly advances" case is a **pre-existing wall-clock flake** (real `DateTime.now()` vs fake time — untouched here). iOS release build + device install ✓; web build ✓; owner-approved on the physical iPhone. **Files:** `moonjoin_top_brand_card.dart`, `moonjoin_food_top_brands.dart`, `moonjoin_campaign_card.dart`, `moonjoin_campaign_section.dart` (new); `item_model.dart` (`store` field), `all_store_screen.dart` + `web_new_home_screen.dart` (imports + gated insertions); `test/moonjoin/campaign_test.dart`; `ui-designs/Top_Brands.png` + `campaign.png`; `pubspec.yaml` (`palette_generator` removed). FROZEN.

## Phase 2 — Shared Top Brands presentation reused across modules (`MoonjoinTopBrandsSection`) — STATUS: FROZEN (2026-08-18) · owner-approved
The approved Food Top Brands visual is now **one shared PRESENTATION component** reused by every mobile Top Brands section — presentation only, each module keeps its own data.

**New shared component.** `moonjoin_top_brands_section.dart` = `MoonjoinTopBrandsSection` (+ `MoonjoinTopBrandItem`), composing the frozen `MoonjoinTopBrandCard`. Owns ONLY visuals (near-square rounded card; vendor/brand image fills via `BoxFit.cover`; exactly four cards across a phone via `cardW = (screenW − 2·pad − 3·gap)/4` clamped 70–120; spacing; corner radius; name-at-bottom; header + optional green "See all"; loading shimmer). Takes `title` + `items` + optional `onSeeAll`; **no controller/API/RouteHelper access** (audited). Self-hides on empty; shimmer while null.

**Migrated (mobile).** Food (`MoonjoinFoodTopBrands` → thin adapter over `featuredStoreList`), Ecommerce/Fashion (`all_store_screen._topBrands()` → `BrandsController.brandList`, See-all `getBrandsScreen`, tap `getBrandsItemScreen`), Rental (`all_vehicle_screen._topBrands()` → `taxiBrandModel.brands`, no See-all, inert brand tap). Data sources remain **independent — never merged/centralized**; no API/controller/model/moduleId/zoneId/filtering/pagination/navigation/backend change.

**Food non-regression.** Shared section uses the identical formula/layout as the frozen Food component → Food is visually identical (golden probe confirmed 4-across near-square + See-all/no-See-all header variants).

**Cleanup.** Removed the legacy duplicate `TopBrandCard` from `all_restaurants_widgets.dart` (used only by the two migrated screens; replaced) + its unused `get` import; breadcrumb comment prevents reintroduction. Other widgets in that file (`RestaurantCategoryChip`/`StoreFilterChip`/`PromotionalBannerCarousel`) kept (still used).

**Deliberate scope.** Mobile only. Web Food already uses the shared component; web/desktop Ecommerce/Rental brand widgets (`WebBrandsViewWidget`, legacy `BrandsViewWidget`) are intentionally out of this phase (desktop strategy undecided). **Approved consequences:** Ecommerce loses the "N+ items" line (approved presentation has none; data untouched); brand logos use `BoxFit.cover` like Food.

**Rejected (recorded):** generated brand colours, palette extraction, `palette_generator`, amoeba/blob cards, floating-logo cards, oversized/portrait cards, fabricated counts, shared cross-module data.

**Verification.** `flutter analyze` clean on all 5 changed files (35 total repo issues = pre-existing, none in these files); 15 deterministic moonjoin tests pass (campaign 9, advertisement 5, flash layout 1); `flash_countdown` "visibly advances" = same pre-existing wall-clock flake (untouched). iOS release build + device install ✓; web build ✓. **Campaign implementation NOT modified.** **Files:** `moonjoin_top_brands_section.dart` (new); `moonjoin_food_top_brands.dart`, `all_store_screen.dart`, `all_vehicle_screen.dart`, `all_restaurants_widgets.dart` (modified). FROZEN.

## Legacy Cleanup — Batch 2: remove proven runtime-dead legacy home code — STATUS: DONE (2026-08-18) · owner-approved
Evidence-based removal of legacy 6amMart code proven **runtime-dead** by the Phase-4 control-flow audit (baseline `013994f`). **No active feature, UI, API, controller, model, navigation or backend touched.**

**Control-flow proof (why dead).** `HomeScreen.body`: mobile Food/Grocery/Pharmacy/Ecommerce → `AllStoreScreen`; desktop → `WebNewHomeScreen`; no-module → `ModuleLandingView`; Taxi → `TaxiHomeScreen`; Parcel → `ParcelCategoryScreen`. The legacy per-module homes were instantiated ONLY in the `CustomScrollView` branch, which is reachable only when `isFood/isGrocery/isPharmacy/isShop` are all false — so their ternary guards can never fire. Confirmed single instantiation site each, no route, no `storefrontMode:true` usage, no test refs.

**Deleted (6 files):** `home/widgets/item_campaign_view.dart` (`ItemCampaignView`/`ItemCampaignShimmer` — zero instantiation sites; superseded by `MoonjoinCampaignSection`), `home/screens/modules/food_home_screen.dart`, `grocery_home_screen.dart`, `shop_home_screen.dart`, `pharmacy_home_screen.dart`, `home/widgets/brands_view_widget.dart` (mobile `BrandsViewWidget`, only consumer was the dead `shop_home_screen`). **Modified:** `home_screen.dart` — removed the 5 now-dead imports and simplified the dead ternary to keep only the active `isTaxi → TaxiHomeScreen` / `SizedBox` leaves.

**KEPT after mid-batch correction (important).** Initially also staged `module_view.dart` and `highlight_widget.dart` for deletion; `flutter analyze` caught that both files ALSO define classes used by ACTIVE code — `module_view.dart` → `AddressShimmer`/`ModuleShimmer` (used by active `deliver_to_view`, `module_landing_view`, Rental `my_address_widget`); `highlight_widget.dart` → `HighlightVideoWidget`/`HighlightStoreWidget`/`AdvertisementIndicator`/`AdvertisementShimmer` (used by active web `WebHighlightWidget`). Both files were **restored**. Their now-unused headline classes `ModuleView` and `HighlightWidget` (mobile) are **dead-but-retained** (can't drop the file) → deferred. Lesson recorded: audit EVERY class in a file, not just the headline class.

**Deferred (not removed):** `ModuleView` + mobile `HighlightWidget` classes (dead, but inside retained files); the UNRESOLVED sub-views (`CategoryView`, `PopularStoreView`, `NewOnMartView`, `RecommendedStoreView`, `BestStoreNearbyView`, `TopOffersNearMe` — duplicate-named files need disambiguation).

**Preserved / not touched:** `AllStoreScreen`, `WebNewHomeScreen`, `ModuleLandingView`, `TaxiHomeScreen`, `ParcelCategoryScreen`, shared Top Brands, Campaign, Flash Sale, banners, categories, store lists, `onRefresh` data-loading, `showMobileModule`/`isDesktop`/module-detection logic, all controllers/repositories/APIs, and web `WebHighlightWidget`/`WebBrandsViewWidget`/`FlashSaleCard`.

**Verification.** `flutter analyze` back to the pre-existing 35 issues, **0 errors, none in changed files** (was momentarily +7 errors with the two over-deletions — fixed by restoring them). 15 deterministic moonjoin tests pass (campaign 9, advertisement 5, flash layout 1); `flash_countdown` flake unchanged. iOS release build + device install ✓; web release build ✓. Net **6 files deleted, −510 lines**.

## Legacy Cleanup — Batch 3 (audit) + Batch 4 (delete orphaned views) — STATUS: DONE (2026-08-18) · owner-approved
Batch 3 was a READ-ONLY reference/dependency audit; Batch 4 removed the 8 files it proved runtime-dead. Baseline `d20824b`. **No active feature, UI, API, controller, model, route, or backend touched.**

**Why dead.** These home "views" were consumed ONLY by the legacy per-module home screens removed in Batch 2, so after Batch 2 they were orphaned (zero importers). Verified: no importers (package/relative/basename), no `export`/barrel, no `part`, no route/DI/factory, no test refs. Two duplicate pairs (`CategoryView`, `PopularStoreView` each existed in `widgets/` and `widgets/views/`) — **both** copies were dead. Same-named classes in active web files (`WebCategoryViewWidget`, `web_populer_store_view_widget`, `category_pop_up`) are SEPARATE self-contained implementations that never imported these files (name collision, not shared usage).

**Deleted (8):** `home/widgets/category_view.dart`, `home/widgets/views/category_view.dart`, `home/widgets/popular_store_view.dart`, `home/widgets/views/popular_store_view.dart`, `home/widgets/views/new_on_mart_view.dart`, `home/widgets/views/recommended_store_view.dart`, `home/widgets/views/best_store_nearby_view.dart`, `home/widgets/views/top_offers_near_me.dart`.

**Modified (1) — `module_view.dart` (surgical):** removed the runtime-dead `ModuleView` class + its ModuleView-only private helpers `_WavyBottomPainter` and `_ScaleTap` + the now-unused imports (including the `popular_store_view.dart` import that had blocked its deletion). **Preserved byte-for-byte** the two ACTIVE classes `ModuleShimmer` (consumed by `module_landing_view.dart`) and `AddressShimmer` (consumed by `deliver_to_view.dart` + Rental `my_address_widget.dart`). File kept under its existing name to minimise risk.

**Safety lesson (from Batch 2, reinforced here).** Never delete an entire file because one class in it is dead — first enumerate EVERY class in the file and check each for active consumers. In Batch 2 this rule saved `module_view.dart`/`highlight_widget.dart` (analyzer caught active shimmer/sub-widget classes); in Batch 4 the same rule meant `module_view.dart` was surgically pruned (not deleted) to keep `ModuleShimmer`/`AddressShimmer`.

**Verification.** `flutter analyze` = 31 issues, **0 errors, 0 new** (down from the pre-existing 35 — the 4 fewer were pre-existing info-lints that lived inside the deleted files). 15 deterministic moonjoin tests pass; `flash_countdown` flake unchanged. iOS release build + device install ✓; web release build ✓. No import references any deleted file; `ModuleView`/`_WavyBottomPainter`/`_ScaleTap` remain only in explanatory comments. Top Brands, Campaign, and all module flows untouched. Net **8 files deleted + 1 pruned, −2517/+9 lines**. Baseline `d20824b` → commit recorded below.

## Legacy Cleanup — Batch 5 (audit) + Batch 6 (delete orphaned home views) — STATUS: DONE (2026-08-18) · owner-approved
Batch 5 was a READ-ONLY audit of the 15 files under `lib/features/home/widgets/views/`; Batch 6 removed the 14 proven runtime-dead. Baseline `ad75729c3b945cefa38e7a0124665806c68a693b`. **No active feature, UI, API, controller, model, route, navigation, or backend touched. No `sixam_mart`/package-identity change.**

**Why dead.** These home "view" widgets were consumed only by the legacy per-module home screens removed in Batch 2, so afterward they were orphaned. Verified: zero external importers (package + relative/basename), no `export`/barrel, no `part`, no RouteHelper/route/binding, no GetX DI, no generated refs, not constructed anywhere active. Same-named classes in active code are SEPARATE files (name collisions, not shared usage): e.g. `ItemShimmerView` is self-defined in the active `item_view_all_screen.dart`; `BannerView` also exists as a different `widgets/banner_view.dart`; the active web equivalents (`WebSpecialOfferView`, `WebVisitAgainView`, `WebJustForYouView`, `WebBestReviewItemViewWidget`, etc.) are distinct live widgets.

**Deleted (14):** `views/banner_view.dart`, `views/best_review_product_view.dart`, `views/best_reviewed_item_view.dart`, `views/common_condition_view.dart`, `views/featured_categories_view.dart`, `views/item_that_you_love_view.dart`, `views/just_for_you_view.dart`, `views/middle_section_banner_view.dart`, `views/most_popular_item_view.dart`, `views/product_with_categories_view.dart`, `views/promotional_banner_view.dart`, `views/special_offer_view.dart`, `views/store_wise_banner_view.dart`, `views/visit_again_view.dart`. (`most_popular_item_view` + `special_offer_view` deleted together — the only intra-set import edge.)

**Retained:** `views/promo_code_banner_view.dart` — its `PromoCodeShimmerView` is used by the active web `web_coupon_banner_view_widget` → `WebNewHomeScreen` (the `PromoCodeBannerView` main class is dead-within → future class-level prune, file kept).
...
## Legacy Cleanup — Batch 8: class-level prune of dead classes inside live files — STATUS: DONE (2026-08-18) · owner-approved
First **class-level** prune (not whole-file deletion): removed runtime-dead classes that live in files whose OTHER classes are active. Baseline `80d268c1aab1c533b8441a68a0e4298ca28a0fc1`. **No active feature, UI, API, controller, model, route, navigation, or backend touched. No `sixam_mart`/package-identity change.**

**Batch-4 lesson applied.** A file containing both dead and active classes must be **pruned at class level**, never deleted wholesale (Batch 2 proved deleting a whole file for one dead class breaks active code). Here `highlight_widget.dart` and `promo_code_banner_view.dart` were surgically pruned; their active classes were preserved byte-for-byte (pure deletions, 0 insertions in those files).

**`highlight_widget.dart` — removed:** `HighlightWidget` + `_HighlightWidgetState` (the legacy mobile Highlights carousel, orphaned by the Batch-2 module-home removal; zero external refs) + the 3 imports only they used (`carousel_slider`, `custom_asset_image_widget`, `util/images`). **Preserved byte-for-byte:** `HighlightStoreWidget`, `HighlightVideoWidget`(+`_HighlightVideoWidgetState`), `AdvertisementIndicator`, `AdvertisementShimmer` — all still consumed by the active web `WebHighlightWidget` → `WebNewHomeScreen`.

**`views/promo_code_banner_view.dart` — removed:** `PromoCodeBannerView` + `_PromoCodeBannerViewState` (zero external refs) + the 9 imports only they used (`dotted_border`, `flutter/services`, `get`, `custom_ink_well`, `coupon_controller`, `price_converter`, `util/images`, `styles`, `custom_snackbar`). **Preserved byte-for-byte:** `PromoCodeShimmerView` (4 imports: `carousel_slider`, `material`, `shimmer_animation`, `dimensions`) — still consumed by the active web `WebCouponBannerViewWidget` → `WebNewHomeScreen`.
...
## Ownership Migration — Dart package identity `sixam_mart` → `moonjoin` (ATOMIC) — STATUS: DONE (2026-08-18) · owner-approved
The CRITICAL package-identity migration: the Dart package name was renamed from the inherited `sixam_mart` to **`moonjoin`**. Rollback checkpoint `bb00aee761ace3fcd1c5b2939947fd607440d98c`. **Presentation/logic-free: no executable logic, API, UI, controller, model, route, navigation, backend, storage-key, notification, Pusher, Firebase, or platform-identity change.**

**Exact change (804 files, +6089/−6089 — pure token swap):**
- `pubspec.yaml:1` `name: sixam_mart` → `name: moonjoin`.
- **790 `lib/` files + 13 `test/` files**: `package:sixam_mart/` → `package:moonjoin/` (6088 import lines, live and commented). Every changed line was proven to be an import token or the pubspec name (0 non-token lines, both directions).

**Explicitly UNCHANGED (proven):** the `6ammart_*` SharedPreferences keys (27 in `app_constants.dart`), `flutter.6ammart_theme` web bridge, notification channel id `'6ammart'` (×3), Pusher key `'6ammart'` (×1), Android `applicationId`/`namespace` `com.moonjoin.com`, iOS `PRODUCT_BUNDLE_IDENTIFIER com.moonjoin.com` (×3) + `CFBundleDisplayName MoonJoin`, `MainActivity` `package com.moonjoin.com`, Firebase, API endpoints, backend, UI, controllers, models, routes. No android/ios/web/config/docs source touched by the token swap. The legacy Android source directory `com/sixamtech/efood_multivendor/` was intentionally left as-is (cosmetic, per prior audit).

**Why safe / contained:** every `sixam_mart` occurrence in lib/test was a `package:sixam_mart/` import (zero non-import refs); no `build_runner`/codegen; no CI; `.dart_tool/package_config.json` regenerated by `flutter pub get`. The stale generated web entrypoint (`.dart_tool/flutter_build/<hash>/main.dart`) that still referenced the old package was cleared via `flutter clean` (generated build cache, not source).

**Verification.** `git grep package:sixam_mart -- lib test` = 0; `package:moonjoin` = 6088 lines / 803 files. `flutter analyze` = 31 issues, 0 errors, 0 new. 15 deterministic moonjoin tests pass. iOS release build + device install ✓ (`databaseSequenceNumber: 21516`); web release build ✓ (after `flutter clean` regenerated the entrypoint); Android release APK ✓ (`app-release.apk`, 55.2MB). Checkpoint `bb00aee` → commit recorded below.
