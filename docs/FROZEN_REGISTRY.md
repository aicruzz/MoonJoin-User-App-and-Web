...
### 🧊 Storefront Browse → Category Items — `category_item_screen.dart` — B1 (header) + B2 (segmented control) — FROZEN
...
- **Still pending on this screen (NOT frozen):** **B3** = sub-category chips → `MoonjoinFilterChip`. Screen remains **Partially Frozen** until B3 is done and approved.
- **Rule:** B1 header + B2 segmented control frozen exactly as owner-approved. Do not modify without explicit approval. Do not touch `ItemsView`/tabs-reload/chips/search-behavior/cart/filters/pagination/business logic.
